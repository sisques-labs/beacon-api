...
8. **Response DTOs never map themselves.** A response DTO exposes fields only — no constructor, no mapping logic. Build it via a dedicated, injected `{Name}RestMapper` (REST) or `{Name}GraphQLMapper` (GraphQL); this applies to both transports identically.
...
│   ├── rest/
│   │   ├── controllers/    {name}.controller.ts       — {Name}Controller, CommandBus/QueryBus only
│   │   ├── mappers/        {name}.mapper.ts            — {Name}RestMapper, toResponseDtoFromResult()/toResponseDtoFromViewModel() — every response DTO with a mutation/write result is built here, never in the DTO's own constructor
│   │   └── dtos/           {name}.dto.ts
│   └── mcp/
...
| REST controller | `{name}.controller.ts`, class `{Name}Controller`, in `transport/rest/controllers/` | `order.controller.ts` |
| REST mapper | `{name}.mapper.ts`, class `{Name}RestMapper`, method `toResponseDtoFromResult()` or `toResponseDtoFromViewModel()`, in `transport/rest/mappers/` | `order.mapper.ts` |
